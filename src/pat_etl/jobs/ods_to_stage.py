"""
ODS->Stage runner (starter).

How to migrate from your ODS-Stage跑批.py:
- Move CLEAN_SQL_TEMPLATE here (or to a .sql template file).
- Replace top-level constants with cfg values.
- Keep your per-file done-table logging logic as-is.

This file is intentionally small so you can learn the structure first.
"""
...
# TODO: paste your INSERT...SELECT template here, or load it from a .sql file.
...
def run_ods_to_stage(cfg: dict):
    ds = get_dataset_cfg(cfg)
    run = get_run_cfg(cfg)
    PRINT_EVERY_FILE_DETAIL = True
    ods_table = ds["ods_table"]
    stage_table = ds["stage_table"]
    table_id = ds.get("table_id", "a")
    batch_id = ds["batch_id"]
    mode = run.get("mode", "all_not_success")
    delete_before_insert = bool(run.get("delete_before_insert", False))
    fetch_files_per_round = int(run.get("fetch_files_per_round", 50))
    commit_every_files = int(run.get("commit_every_files", 5))
    degrade_to_single_on_group_fail = bool(run.get("degrade_to_single_on_group_fail", True))

    print(f"[ods2stage] ods={ods_table} -> stage={stage_table} batch_id={batch_id} table_id={table_id}")
    print("TODO: move logic from ODS-Stage跑批.py into this function.")
